...
class BacktestEngine:
    ...
    def _run_strategy(
        ...
        for i in range(1, len(portfolio)):
            ...
            if abs(position_change) > 0.001:  # Only trade if significant change
                ...
                new_cash = (
                    current_cash
                    - (shares_to_trade * effective_price)
                    - transaction_cost
                )
            ...
            portfolio.loc[i, "cumulative_returns"] = (
    ...
    def _calculate_performance_metrics(
        self, portfolio: pd.DataFrame, strategy_name: str
    ) -> Dict:
...
class StrategyBuilder:
    ...
    def macd_crossover_strategy(
        df: pd.DataFrame, macd_col: str = "macd", macd_signal_col: str = "macds"
    ) -> pd.Series:
        """
    ...
    for period_name, (start_date, end_date) in stress_periods:
        try:
            ...
            result = engine.run_backtest(
                period_df, period_signals, strategy_name=f"stress_{period_name}"
            )
