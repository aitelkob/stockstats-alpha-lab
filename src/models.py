...
class ModelPipeline:
    ...
    def train_model(
        ...
        if hasattr(
            pipeline.named_steps[list(pipeline.named_steps.keys())[-1]], "predict_proba"
        ):
    ...
    def _calculate_metrics(
        ...
        is_classification = (
            y_true.dtype in ["int64", "int32", "bool"] or len(np.unique(y_true)) < 10
        )
    ...
    def _extract_feature_importance(
        self, pipeline: Pipeline, model_name: str, feature_names: List[str]
    ) -> None:
        """Extract feature importance from trained model."""
        try:
            # Get the final estimator
            final_estimator = pipeline.named_steps[
                list(pipeline.named_steps.keys())[-1]
            ]
    ...
    def cross_validate_model(
        self, model_name: str, X: pd.DataFrame, y: pd.Series, cv_folds: int = 5
    ) -> Dict:
        """
        ...
        is_classification = (
            y.dtype in ["int64", "int32", "bool"] or len(np.unique(y)) < 10
        )
        ...
        logger.info(
            f"CV results for {model_name}: {np.mean(cv_scores):.4f} ± {np.std(cv_scores):.4f}"
        )
        return results
...
def calculate_information_coefficient_ml(
    y_true: pd.Series, y_pred: pd.Series, method: str = "spearman"
) -> float:
    """
