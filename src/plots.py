...
class Plotter:
    ...
    def plot_price_and_indicators(
        ...
        Returns:
            Matplotlib figure
        """
        fig, axes = plt.subplots(len(indicators) + 1, 1, figsize=self.figsize, dpi=self.dpi)
        fig.suptitle(title, fontsize=16, fontweight='bold')
        
        # Plot price
        axes[0].plot(df.index, df['close'], color=self.colors['price'], linewidth=2, label='Close Price')
        ...
        for i, indicator in enumerate(indicators):
            if indicator in df.columns:
                axes[i + 1].plot(df.index, df[indicator], color=self.colors['indicator'], linewidth=1.5)
    ...
    def plot_backtest_results(
        ...
        # Equity curve
        axes[0, 0].plot(portfolio.index, portfolio['cumulative_returns'], 
                       color=self.colors['price'], linewidth=2, label='Strategy')
        
        if benchmark_returns is not None:
            benchmark_cumulative = (1 + benchmark_returns).cumprod() - 1
            axes[0, 0].plot(portfolio.index, benchmark_cumulative, 
                           color=self.colors['neutral'], linewidth=2, label='Benchmark')
        ...
        # Drawdown
        axes[0, 1].fill_between(portfolio.index, portfolio['drawdown'], 0, 
                               color=self.colors['loss'], alpha=0.7, label='Drawdown')
        ...
        axes[1, 0].hist(returns, bins=50, alpha=0.7, color=self.colors['indicator'], edgecolor='black')
        axes[1, 0].axvline(returns.mean(), color=self.colors['signal'], linestyle='--', 
        ...
        axes[1, 1].plot(portfolio.index, rolling_sharpe, color=self.colors['indicator'], linewidth=1.5)
    ...
    def plot_strategy_comparison(
        ...
        for i in range(len(plot_data.index)):
            for j in range(len(metrics)):
                text = ax.text(i, j, f'{plot_data.iloc[i, j]:.3f}',
    ...
    def plot_correlation_heatmap(
        ...
        if columns is None:
            ...
            columns = [col for col in numeric_cols if col not in ['open', 'high', 'low', 'close', 'volume']]
        ...
        ax.set_xticklabels([col.replace('_', ' ').title() for col in columns], 
    ...
    def create_tearsheet(
        ...
        ax1.plot(portfolio.index, portfolio['cumulative_returns'], 
                color=self.colors['price'], linewidth=2, label='Strategy')
        
        if benchmark_returns is not None:
            benchmark_cumulative = (1 + benchmark_returns).cumprod() - 1
            ax1.plot(portfolio.index, benchmark_cumulative, 
                    color=self.colors['neutral'], linewidth=2, label='Benchmark')
        ...
        ax2.fill_between(portfolio.index, portfolio['drawdown'], 0, 
        ...
        ax3.hist(returns, bins=50, alpha=0.7, color=self.colors['indicator'], edgecolor='black')
        ax3.axvline(returns.mean(), color=self.colors['signal'], linestyle='--', 
                   label=f'Mean: {returns.mean():.4f}')
        ...
        rolling_sharpe = returns.rolling(252).mean() / returns.rolling(252).std() * np.sqrt(252)
        ax4.plot(portfolio.index, rolling_sharpe, color=self.colors['indicator'], linewidth=1.5)
