...
class TestIndicators:
    ...
    def test_add_basic_indicators(self, sample_data):
        ...
        for indicator in expected_indicators:
            assert indicator in df_with_indicators.columns, f"Indicator {indicator} not found"
    ...
    def test_data_quality_after_indicators(self, sample_data):
        ...
        for col in numeric_cols:
            assert not np.isinf(df_with_indicators[col]).any(), f"Infinite values found in {col}"
        ...
        for col in numeric_cols:
            if col not in ['open', 'high', 'low', 'close', 'volume']:
                ...
                assert nan_counts[col] < len(df_with_indicators) * 0.5, f"Too many NaN values in {col}"
