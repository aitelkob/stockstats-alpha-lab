...
class TestLabeling:
        ...
        data = pd.DataFrame(
            {
                ...
                "close_20_sma": prices
                * (1 + np.random.normal(0, 0.05, 100)),  # Changed from close_200_sma
            },
            index=dates,
        )
    ...
    def test_forward_return_label(self, sample_data):
        ...
        # Test log returns
        forward_returns = labeler.forward_return_label(
            sample_data, horizon=5, return_type="log"
        )
    ...
    def test_forward_return_types(self, sample_data):
        ...
        # Test simple returns
        simple_returns = labeler.forward_return_label(
            sample_data, horizon=5, return_type="simple"
        )
        assert len(simple_returns) == len(sample_data)

        # Test excess returns
        excess_returns = labeler.forward_return_label(
            sample_data, horizon=5, return_type="excess"
        )
        ...
        if valid_idx.any():
            assert (
                simple_returns[valid_idx] > np.log(1 + simple_returns[valid_idx])
            ).all()
    ...
    def test_binary_classification_label(self, sample_data):
        ...
        binary_labels = labeler.binary_classification_label(
            forward_returns, threshold=0.0
        )
    ...
    def test_multi_class_label(self, sample_data):
        ...
        multi_labels = labeler.multi_class_label(
            forward_returns, quantiles=[0.25, 0.75]
        )
    ...
    def test_create_feature_matrix_with_lookback(self, sample_data):
        ...
        X, y = create_feature_matrix(
            sample_data, indicator_cols, forward_returns.name, lookback_window=3
        )
    ...
    def test_transaction_cost_impact(self, sample_data):
        ...
        labels_with_cost = labeler.binary_classification_label(
            forward_returns, transaction_cost=0.01
        )
        labels_without_cost = labeler.binary_classification_label(
            forward_returns, transaction_cost=0.0
        )
